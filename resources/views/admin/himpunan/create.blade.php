...
@section('content')
  ...
  <div class="col-lg-12 col-lg-12 form-wrapper" id="tambah-himpunan">
    ...
    @if(auth()->user()->roles_id == 1)
        <form method="POST" action="{{ route('super.himpunan.store',$himpunan->id) }}" enctype='multipart/form-data'>
    @elseif(auth()->user()->roles_id == 2)
        <form method="POST" action="{{ route('admin.himpunan.store',$himpunan->id) }}" enctype='multipart/form-data'>
    @elseif(auth()->user()->roles_id == 3)
        <form method="POST" action="{{ route('himpunans.himpunan.store',$himpunan->id) }}" enctype='multipart/form-data'>
    @endif
        @csrf
          <div class="mb-3 row">
